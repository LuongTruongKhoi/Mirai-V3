...
module.exports.run = async ({ api, event, Threads }) => {
  ...
  api.sendMessage({body: `==== [ 𝗪𝗢𝗥𝗞 𝗞𝗜𝗘̂́𝗠 𝗧𝗜𝗘̂̀𝗡 ] ====\n━━━━━━━━━━━━━━━━\n\n${Object.entries(works).map(($, i)=> `[ ${i+1} | ${$[0]} ] ${$[1].name}`).join('\n')}\n\n➩ 𝗥𝗲𝗽𝗹𝘆 𝘁𝗵𝗲𝗼 𝘁𝗵𝘂̛́ 𝘁𝘂̛̣ 𝗼𝗿 𝘁𝗵𝗮̉ 𝗰𝗮̉𝗺 𝘅𝘂́𝗰 𝘁𝘂̛𝗼̛𝗻𝗴 𝘂̛́𝗻𝗴 𝘃𝗮̀𝗼 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘃𝗼̛́𝗶 𝗻𝗵𝘂̛̃𝗻𝗴 𝗰𝗼̂𝗻𝗴 𝘃𝗶𝗲̣̂𝗰 𝘁𝗿𝗲̂𝗻 đ𝗲̂̉ 𝗸𝗶𝗲̂́𝗺 𝘁𝗶𝗲̂̀𝗻`, attachment: (img)}, event.threadID, (err, res) => (res.name = exports.config.name, res.event = event, global.client.handleReaction.push(res), global.client.handleReply.push(res)), event.messageID);
    }
exports.handleReaction = async o => {
    ...
    let wgm = await new Promise(async resolve => send({body: `Đang ${work.name}...`,
